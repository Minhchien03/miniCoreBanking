...
async def create_account(
    ...
    logger.info(f"Request to create account for owner: {request.owner_name}")
    ...
    logger.info(
        f"Created new account with ID: {new_account.id} for owner: {new_account.owner_name}"
    )
...
async def deposit_funds(
    ...
    logger.info(
        f"Request to deposit {request.amount} to account {request.account_id}"
    )

    async with db.begin():
        stmt = select(Account).where(
            Account.ma_khach_hang_fake == request.account_id
        )
        account = (await db.execute(stmt)).scalar_one_or_none()

        if not account:
            logger.warning(
                f"Account {request.account_id} not found for deposit"
            )
    ...
    logger.info(
        f"Deposit of {request.amount} to account {request.account_id} successful, transaction ID: {new_txn.id}"
    )
...
async def transfer_funds(
    ...
    if request.sender_id == request.receiver_id:
        logger.warning(
            f"Transfer request with same sender and receiver: {request.sender_id}"
        )
        raise HTTPException(
            status_code=400,
    ...
    async with db.begin():
        ...
        logger.info(
            f"Locking accounts for transfer: sender {request.sender_id}, receiver {request.receiver_id}"
        )
        ...
        if not sender:
            logger.warning(
                f"Sender account {request.sender_id} not found for transfer"
            )
            raise HTTPException(
        ...
        if not receiver:
            logger.warning(
                f"Receiver account {request.receiver_id} not found for transfer"
            )
            raise HTTPException(
        ...
        if current_balance < request.amount:
            logger.warning(
                f"Insufficient funds for transfer: sender {request.sender_id}, balance {current_balance}, attempted transfer {request.amount}"
            )
            raise HTTPException(
    ...
    logger.info(
        f"Transfer successful: {request.amount} from account {request.sender_id} to account {request.receiver_id}, transaction ID: {new_txn.id}"
    )
